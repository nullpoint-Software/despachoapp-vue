...
const progress = computed(() =>
  props.steps.length ? `${index.value + 1} / ${props.steps.length}` : '0 / 0'
)
        ...
        >
          ...
          <footer>
            ...
            <div>
              ...
              <button type="button" class="app-tutorial__next" @click="next">
...
.app-tutorial__spotlight {
  position: fixed;
  z-index: 0;
  border: 2px solid var(--br-accent);
  background: transparent;
  box-shadow:
    0 0 0 9999px color-mix(in srgb, var(--br-bg) 84%, transparent),
    8px 8px 0 var(--br-accent);
  pointer-events: none;
  transition:
    left 0.2s ease,
    top 0.2s ease,
...
.app-tutorial__card > header > div {
  padding: 1rem 1.1rem;
}
.app-tutorial__card header p {
  margin: 0 0 0.35rem;
  color: var(--br-accent) !important;
  font: 800 0.62rem var(--font-family);
  letter-spacing: 0.1em;
  text-transform: uppercase;
}
.app-tutorial__card header strong {
  display: block;
  color: var(--br-text) !important;
  font: 900 1.35rem/1.05 var(--font-family);
  text-wrap: balance;
}
.app-tutorial__card header button {
  border: 0;
...
.app-tutorial__body > span {
  color: var(--br-accent) !important;
  font: 900 0.65rem var(--font-family);
}
.app-tutorial__body p {
  max-width: 58ch;
  margin: 0.65rem 0 0;
  color: var(--br-text) !important;
  font: 600 0.86rem/1.55 var(--font-family);
  text-wrap: pretty;
}
.app-tutorial__card > footer {
  display: flex;
  align-items: center;
...
.app-tutorial__card footer > div {
  display: flex;
  gap: 0.55rem;
}
.app-tutorial__card footer button {
  min-height: 2.65rem;
  border: 1px solid var(--br-line-strong);
  background: transparent;
  color: var(--br-text);
  padding: 0.55rem 0.8rem;
  font: 800 0.65rem var(--font-family);
  text-transform: uppercase;
  cursor: pointer;
}
.app-tutorial__card footer button:disabled {
  opacity: 0.55;
}
.app-tutorial__card footer .app-tutorial__next {
...
@media (max-width: 560px) {
  .app-tutorial__card {
    ...
    box-shadow: 5px 5px 0 var(--br-accent);
  }
  .app-tutorial__card > footer {
    align-items: stretch;
