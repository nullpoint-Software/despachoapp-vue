interface DateTimePickerProps {modelValue:Date;showTime?:boolean;modal?:boolean;title?:string}

interface DateTimePickerEmits {(event:"update:modelValue",value:Date):void}

import { computed, ref, watch } from "vue";

const props=withDefaults(defineProps<DateTimePickerProps>(),{showTime:true,modal:false,title:"Seleccionar fecha"});
...
function dayLabel(day:number){return new Intl.DateTimeFormat("es-MX",{dateStyle:"full"}).format(new Date(draft.value.getFullYear(),draft.value.getMonth(),day))}
