...
export default defineComponent({
  ...
  inheritAttrs: false,
  props: {
    value: { type: Array as PropType<Record<string, any>[]>, default: () => [] },
  ...
  setup(props, { slots, attrs }) {
    ...
    provide(appTableKey, {
      register(column) {
        if (!columns.some((item) => item.id === column.id)) columns.push(column)
      },
      update(id, column) {
        const index = columns.findIndex((item) => item.id === id)
        if (index >= 0) Object.assign(columns[index], column)
      },
      unregister(id) {
        const index = columns.findIndex((item) => item.id === id)
    ...
    const page = ref(0),
      perPage = ref(props.rows),
      sortField = ref(''),
      sortDirection = ref<1 | -1>(1)
    const query = computed(() =>
      String(props.filters?.global?.value ?? '')
        .trim()
    ...
    const filtered = computed(() => {
      const rows = !query.value
        ? [...props.value]
        : props.value.filter((row) =>
            props.globalFilterFields.some((field) =>
              String(row?.[field] ?? '')
                .toLocaleLowerCase('es')
      ...
      return rows.sort(
        (a, b) =>
          String(a?.[sortField.value] ?? '').localeCompare(
            String(b?.[sortField.value] ?? ''),
            'es',
            { numeric: true }
          ) * sortDirection.value
      )
    ...
    watch([query, perPage], () => (page.value = 0))
    watch(pageCount, (count) => {
      if (page.value >= count) page.value = count - 1
    })
    const toggleSort = (column: AppColumnConfig) => {
      if (!column.sortable || !column.field) return
      if (sortField.value === column.field) {
        if (sortDirection.value === 1) sortDirection.value = -1
    ...
    const pagerButton = (icon: string, label: string, disabled: boolean, action: () => void) =>
      h(
        'button',
        {
          type: 'button',
