  ...
  setup(props,{slots,attrs}){
    ...
    return()=>h("section",{...attrs,class:["app-data-table",attrs.class]},[
      ...
      h("div",{class:"app-data-table__scroll"},h("table",{class:"app-data-table__table"},[
        ...
        h("tbody",{},visibleRows.value.length?visibleRows.value.map((row,index)=>h("tr",{key:row.id??row.id_cliente??index,class:props.rowClass?.(row,index)},columns.map(column=>{const rawValue=String(row?.[column.field||""]??"");const customBody=column.slots.body?.({data:row});return h("td",{key:String(column.id),title:customBody?undefined:(rawValue||undefined)},h("div",{class:"app-data-table__cell"},customBody??rawValue))}))):h("tr",{},h("td",{colspan:Math.max(1,columns.length),class:"app-data-table__empty"},"No hay registros para mostrar")))
      ])),
