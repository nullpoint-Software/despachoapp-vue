...
async function applyVisualMarkdown(command: MarkdownCommand) {
  ...
  switch (command) {
    ...
    case "ordered":
      ...
      break;
    case "checklist":
      insertVisualHtml('<ul class="task-list"><li><input type="checkbox"> Pendiente</li><li><input type="checkbox" checked> Completado</li></ul><p><br></p>');
      break;
    case "quote":
    ...
    case "image": {
      const url = await promptDialog({
        ...
        message: "Pega la URL. La descripci\u00f3n aparecer\u00e1 junto a la imagen y podr\u00e1s editarla.",
      ...
      insertVisualHtml('<figure class="document-image"><img src="' + safeUrl + '" alt="' + description + '" contenteditable="false"><figcaption data-placeholder="Descripci\u00f3n de la imagen">' + description + '</figcaption></figure><p><br></p>');
      break;
    }
    case "table":
      insertVisualHtml('<table><thead><tr><th>Columna 1</th><th>Columna 2</th></tr></thead><tbody><tr><td>Dato 1</td><td>Dato 2</td></tr><tr><td>Dato 3</td><td>Dato 4</td></tr></tbody></table><p><br></p>');
      break;
    case "code": {
  ...
  if (["b", "i", "u", "k"].includes(command)) {
    ...
    const mapped: MarkdownCommand =
      command === "b" ? "bold"
        : command === "i" ? "italic"
          : command === "u"
            ? event.shiftKey ? "removeUnderline" : "underline"
            : "link";
...
function replaceEditorSelection(
  ...
  const cursorStart =
    selection.start + (marker >= 0 ? marker : replacement.length);
...
function applyMarkdown(command: MarkdownCommand) {
  ...
  switch (command) {
    ...
    case "heading3": {
      ...
      replaceEditorSelection(
        selection,
        blockSpacing(selection, prefix + text),
        text,
      );
      break;
    }
...
function plain(value:string){return String(value||"").replace(/<[^>]+>/g,"").replace(/[#*_>`\[\]]/g,"").slice(0,95)||"Documento sin contenido"}
...
function createNote(){const target=currentPath.value.startsWith("__")?"General":normalizePath(currentPath.value);editing.value=null;draft.value={titulo:"",descripcion:"",folderPath:target};resetEditorHistory();previewZoom.value=100;editorMode.value="preview";editorOpen.value=true;syncVisualFromMarkdown()}
function openNote(note:Note){editing.value=note;draft.value={titulo:note.titulo,descripcion:note.descripcion,folderPath:normalizePath(note.folderPath||"General")};resetEditorHistory();previewZoom.value=100;editorMode.value="preview";editorOpen.value=true;syncVisualFromMarkdown()}
function closeEditor(){if(!saving.value)editorOpen.value=false}
async function saveEditor(){if(editorMode.value!=="write")syncMarkdownFromVisual();if(!draft.value.titulo.trim())return;saving.value=true;const path=normalizePath(draft.value.folderPath);if(path!=="General"){folders.value.push(...folderAncestors(path));saveFolders()}try{if(editing.value)await updateNote(editing.value.id,{...draft.value,folderPath:path});else await addNote({...draft.value,folderPath:path,pinned:false,color:"white",status:"canvas"});expandPath(path);currentPath.value=path;editorOpen.value=false}finally{saving.value=false}}
