...
async function loadTasks(page = currentPage.value): Promise<void> {
  ...
  try {
    ...
    tasks.value = uniqueById(pageTasks).filter(canCurrentUserSee).map(normalizeTask)
...
const filteredTasks = computed<KanbanTask[]>(() => {
  ...
  return tasks.value.filter((task) =>
    [task.titulo, task.descripcion, task.nombre, task.username, task.estado].some((value) =>
      String(value ?? '')
...
async function moveTask(taskId: number | string, targetStatus: KanbanStatus): Promise<void> {
  ...
  const assignedUser =
    targetStatus === 'Disponible' ? null : (task.id_usuario ?? (userId ? Number(userId) : null))

  task.estado = targetStatus
  task.id_usuario = assignedUser
  task.nombre =
    targetStatus === 'Disponible' ? null : (task.nombre ?? localStorage.getItem('fullname'))
  task.username =
    targetStatus === 'Disponible' ? null : (task.username ?? localStorage.getItem('username'))
  task.image =
    targetStatus === 'Disponible' ? null : (task.image ?? localStorage.getItem('userphoto'))
  task.fecha_vencimiento = targetStatus === 'Terminado' ? new Date().toLocaleString('sv-SE') : null
...
async function openTaskForm(mode: 'add' | 'edit', task: KanbanTask | null = null): Promise<void> {
  const permission =
    mode === 'edit' ? await hasPermission('canEditCard') : await hasPermission('canAddCard')
...
async function openReportPreview(): Promise<void> {
  ...
  try {
    ...
    const result = await prepareTaskReportPreview(
      reportTasks,
      reportDate.value.toLocaleDateString('sv-SE'),
      {
        userId: localStorage.getItem('userid'),
        responsibleName:
          localStorage.getItem('fullname') || localStorage.getItem('username') || 'Usuario'
      }
    )
