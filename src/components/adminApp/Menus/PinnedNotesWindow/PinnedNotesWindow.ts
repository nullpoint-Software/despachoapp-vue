...
const { pinnedNotes: storedPinnedNotes, isPinnedWindowVisible, togglePinnedWindow, updateNote } = useNotesStore();
...
const filteredNotes = computed(() => {
  ...
  const matches = term ? storedPinnedNotes.value.filter(note => normalize(`${note.titulo} ${note.descripcion} ${note.folderPath || ""}`).includes(term)) : storedPinnedNotes.value;
