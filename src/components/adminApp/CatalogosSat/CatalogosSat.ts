...
let activeStoredLookup: { query: string; promise: Promise<IndexedRecommendation | null> } | null =
  null
...
const classificationStatusText = computed(() => {
  if (isClassifying.value) {
    if (classificationPhase.value === 'understanding') {
      return (
        'Gemini Nano está identificando “' + classifyingQuery.value + '” en este dispositivo...'
      )
    }
...
const categoryUsage = computed(() => {
  ...
  const descendants = prefix
    ? catalog.value
        .filter(
          (record) => record.i !== code && record.i.startsWith(prefix) && !record.i.endsWith('00')
        )
...
async function loadCatalog(force = false) {
  ...
  try {
    ...
    error.value =
      catalogError instanceof Error ? catalogError.message : 'Ocurrio un error inesperado.'
  } finally {
    loading.value = false
  }
}
...
function recommendationPriority(recommendation: IndexedRecommendation) {
  return (
    (recommendation.status === 'confirmed' ? 4 : 0) + (recommendation.source === 'learned' ? 2 : 0)
  )
}
...
function indexRecommendation(
  ...
  const selectedRecommendation =
    previous && recommendationPriority(previous) > recommendationPriority(incoming)
      ? { ...previous, aliases: Array.from(new Set([...previous.aliases, ...incoming.aliases])) }
      : {
          ...incoming,
          aliases: Array.from(new Set([...(previous?.aliases || []), ...incoming.aliases]))
        }
  ...
  for (const phrase of recommendationPhrases(selectedRecommendation)) {
    ...
    if (
      ...
      recommendationPriority(selectedRecommendation) >=
        recommendationPriority(previousRecommendation)
    ) {
...
function findIndexedRecommendation(value: string): IndexedRecommendation | null {
  ...
  const ranked = Array.from(candidateKeys)
    ...
    .filter(
      (
        entry
      ): entry is { recommendation: IndexedRecommendation; matchedTokens: number; score: number } =>
        Boolean(entry)
    )
...
async function preflightStoredRecommendation(product: string) {
  ...
  try {
    ...
    if (
      lookupSequence !== storedLookupSequence ||
      normalizeSatText(query.value) !== normalizeSatText(product)
    ) {
      return indexed
    }
...
function applyIndexedRecommendation(
  ...
  searchResults.value = [
    ...
    ...searchResults.value.filter(
      (record) => !associatedRecords.some((associated) => associated.i === record.i)
    )
  ]
...
function scoreRecord(record: SatSearchRecord, normalizedQuery: string, tokens: string[]) {
  const codeQuery = normalizedQuery.replace(/\s/g, '')
  if (record.i === codeQuery) return 1000
  let score = record.i.startsWith(codeQuery) && /^\d+$/.test(codeQuery) ? 700 : 0
...
function searchCatalog() {
  ...
  searchResults.value = catalog.value
    ...
    .filter((entry) => entry.score >= 0)
    .sort((a, b) => b.score - a.score || a.record.d.localeCompare(b.record.d, 'es'))
    .slice(0, 200)
    .map((entry) => entry.record)
...
function directProductContext(product: string): ProductContext | null {
  ...
  const hasDirectMatch = searchResults.value.slice(0, 12).some((record) => {
    const searchable = `${record.normalizedDescription} ${record.normalizedSimilar}`
    return (
      searchable.includes(normalizedProduct) ||
...
function recommendationCandidates(
  ...
  const contextText = normalizeSatText(
    `${product} ${context.summary} ${context.searchTerms.join(' ')}`
  )
  const snackBrand =
    /\b(sabritas|doritos|cheetos|tostitos|takis|ruffles|fritos|rancheritos|churrumais)\b/
  const isSnack = /botana|snack|aperitivo|fritura/.test(contextText) || snackBrand.test(contextText)
  const isHumanFood =
    isSnack || /consumo humano|alimento procesado|comestible|dulce|bebida/.test(contextText)
  const nonHumanFood =
    /maquina|equipo|escurridor|exhibidor|almacenador|expendedor|animal|mascota|ave|perro|gato|forraje|semilla|plantula|cultivo/
      .source
  ...
  return Array.from(
    new Map([...ranked.slice(0, 6), ...literalResults].map((record) => [record.i, record])).values()
  ).slice(0, 8)
}
...
async function classifyCurrentProduct() {
  ...
  try {
    ...
    if (localRecommendation && applyIndexedRecommendation(product, localRecommendation, 'local'))
      return
    ...
    if (!candidates.length) {
      throw new Error(
        'Nano identificó el producto, pero no encontró una clave oficial candidata suficientemente cercana.'
      )
    }
    ...
    if (primaryRecord) {
      ...
      void queueNanoExamples(primaryRecord).catch((exampleFailure) => {
        examplesError.value =
          exampleFailure instanceof Error
...
function officialChildrenFor(record: SatSearchRecord) {
  ...
  return catalog.value
    .filter(
      (candidate) =>
        candidate.i !== record.i && candidate.i.startsWith(prefix) && !candidate.i.endsWith('00')
    )
    .slice(0, 24)
...
async function autoCompleteAndSaveManualProduct() {
  ...
  try {
    ...
    const catalogTerms = record.s
      .split(/[,;]/)
      .map((term) => term.trim())
      .filter(Boolean)
    manualAliases.value = splitManualList(
      [...contextTerms, ...catalogTerms.slice(0, 4)].join(', '),
      10
    )
    ...
    const reusedExistingRelation =
      lastClassificationOrigin.value === 'local' || lastClassificationOrigin.value === 'server'
    ...
    manualMessage.value = reusedExistingRelation
      ...
      : cachedExamples
        ? 'Relación ' +
          record.i +
          ' y ' +
          cachedExamples.examples.length +
    ...
    manualMessage.value =
      analysisError instanceof Error ? analysisError.message : 'Nano no pudo completar la relación.'
    manualState.value = 'error'
  } finally {
...
async function exportRecommendationJson() {
  ...
  try {
    ...
    anchor.href = url
    anchor.download =
      'despachoapp-recomendaciones-sat-' + new Date().toISOString().slice(0, 10) + '.json'
    anchor.click()
...
function toggleFavorite(code: string) {
  ...
  localStorage.setItem('sat-product-favorites', JSON.stringify(favorites.value))
}
