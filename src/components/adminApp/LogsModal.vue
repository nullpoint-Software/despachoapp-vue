      ...
      <div class="modal-content bg-gray-50 p-6 rounded-md shadow-lg w-full max-w-3xl mt-10 max-h-[90vh] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto pr-2">
          <div v-for="(log, i) in filteredLogs" :key="log.id" @click="toggleDetails(i)"
            ...
            <div class="flex justify-between items-center">
              <div>
                <p class="font-semibold text-black">
                  {{ humanizeType(log.type) + " por " + getUser(log) }}
                </p>
              ...
              <div class="flex items-center space-x-2">
                <i id="mostrar" class="pi pi-chevron-down text-xl text-gray-700"></i>
                <Button id="undo" icon="pi pi-undo" title="Deshacer cambio"
                  class="p-button-text p-button-md text-gray-700 hover:text-gray-500" @click.stop="onUndo(log.id)"
                  aria-label="Deshacer cambio">
                </Button>
              </div>
            </div>
            <div v-if="detailsVisible[i]" class="mt-4 w-full text-black">
              <p v-if="log.payload" class="text-2xl text-center">
                Datos actuales
              </p>
            ...
            <div v-if="detailsVisible[i]" class="mt-4 w-full text-black">
              <p v-if="log.oldpayload" class="text-2xl text-center">
                Datos anteriores
              </p>
...
const logs = ref(
  rawLogs
    .map((log) => ({
      ...
      oldpayload:
        typeof log.oldpayload === "string"
          ? JSON.parse(log.oldpayload)
          : log.oldpayload,
...
const filteredLogs = computed(() =>
  logs.value.filter((log) => {
    ...
    if (selectedTypes.value.length && !selectedTypes.value.includes(suffix))
      return false;
    
    if (!showAll.value) {
      if (
        currentPath.includes("/app/pagos/mensual") &&
        !log.type.startsWith("PagoMensual")
      ) {
        currentPageTitle.value = "Pago Mensual"
        return false;
      }
      if (
        currentPath.includes("/app/pagos/concepto") &&
        !log.type.startsWith("PagoConcepto")
      ) {
        currentPageTitle.value = "Pago Concepto"
        return false;
      }
      if (
        currentPath.includes("/app/pagos") &&
        !log.type.startsWith("Pago")
      ) {
        currentPageTitle.value = "Pagos"
        return false;
      }

      if (
        currentPath.includes("/app/clientes") &&
        !log.type.startsWith("Cliente")
      ) {
        currentPageTitle.value = "Clientes"
        return false;
      }
      if (currentPath.includes("/app/tareas") && !log.type.startsWith("Tarea")) {
        currentPageTitle.value = "Tareas"
        return false;
      }
    }else{
      
      currentPageTitle.value = "Todos"
    }
    return true;
...
function toggleDetails(i) {
  detailsVisible.value[i] = !detailsVisible.value[i];
}
function closeModal() {
  emit("close");
...
async function onUndo(id) {
  const selectedLog = logs.value.find((l) => l.id == id);
  console.log("undo");
  try {
...
function getUser(log) {
  const index = users.findIndex((u) => u.id_usuario == log.userid);
