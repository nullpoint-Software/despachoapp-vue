interface ThirdPartyComplianceModalEmits {
  close: []
...
function errorText(error: unknown, fallback: string) {
  ...
  if (candidate?.response?.status === 404)
    return 'El servidor remoto todavía no tiene instalada la actualización de consulta por terceros.'
  if (candidate?.code === 'ECONNABORTED')
    return 'El servidor remoto tardó demasiado en responder. Reinicia el backend con la imagen actualizada.'
  if (!candidate?.response && candidate?.message === 'Network Error')
    return 'No hay conexión con el servidor remoto.'
  return fallback
}
...
async function submitNativeLogin() {
  ...
  errorMessage.value = ''
  try {
    state.value = await cos.iniciarAccesoTerceros(sessionId.value, {
      rfc: loginRfc.value,
...
function clickViewport(event: MouseEvent) {
  ...
  queueInput({
    type: 'click',
    x: ((event.clientX - rect.left) * state.value.width) / rect.width,
    y: ((event.clientY - rect.top) * state.value.height) / rect.height
  })
...
function sendTypedText() {
  textBuffer += typedText.value
  typedText.value = ''
  if (textTimer) clearTimeout(textTimer)
  textTimer = setTimeout(flushText, 110)
}
  ...
  try {
    ...
    while (hasMore) {
      ...
      progress.value = {
        ...
        documents: progress.value.documents + response.documents
      }
      if (response.processed <= offset && response.hasMore)
        throw new Error('La consulta no avanzó.')
      offset = response.processed
      hasMore = response.hasMore
    }
    emit(
      'complete',
      `Consulta por terceros terminada: ${progress.value.processed} clientes revisados y ${progress.value.documents} PDF actualizados.`
    )
    await close(false)
  } catch (error) {
