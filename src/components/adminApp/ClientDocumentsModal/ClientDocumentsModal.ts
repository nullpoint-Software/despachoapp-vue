...
interface ClientDocumentsModalEmits {
  (event: 'close'): void
}
...
onMounted(() => {
  if (hasSensitiveAccess()) void unlockWithActiveAccess()
})
...
async function unlockWithActiveAccess() {
  if (!hasSensitiveAccess()) return
  verifying.value = true
  error.value = ''
  try {
    const response = await cs.verificarDocumentos(Number(props.client.id_cliente), '')
    accessToken.value = response.accessToken
    await load()
  } catch (requestError: any) {
    clearSensitiveAccess()
    if (requestError?.response?.data?.code !== 'SENSITIVE_ACCESS_EXPIRED') {
      error.value = requestError.response?.data?.error || 'No fue posible abrir el expediente.'
    }
  } finally {
...
async function unlock() {
  ...
  error.value = ''
  try {
    const response = await cs.verificarDocumentos(
      Number(props.client.id_cliente),
...
async function download(document: ClientDocument) {
  ...
  error.value = ''
  try {
    const blob = await cs.descargarDocumento(
      Number(props.client.id_cliente),
      document.id,
      accessToken.value
    )
    ...
    toast.add({
      severity: 'success',
      summary: 'Documento preparado',
      detail: document.nombre_original,
      life: 2500
    })
  } catch (requestError: any) {
    if (lockOnExpired(requestError)) return
