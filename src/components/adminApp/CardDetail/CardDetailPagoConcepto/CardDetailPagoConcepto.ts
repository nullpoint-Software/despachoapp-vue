...
const paymentSteps: Array<{ index: PaymentStep; eyebrow: string; label: string; description: string }> = [
  { index: 1, eyebrow: "01", label: "Relación", description: "Selecciona el cliente, el concepto y quién atendió el movimiento." },
  { index: 2, eyebrow: "02", label: "Movimiento", description: "Captura los importes y la fecha real del movimiento." },
  { index: 3, eyebrow: "03", label: "Confirmación", description: "Revisa el resumen antes de guardar el pago." },
];
const stepDescription = computed(() => paymentSteps[currentStep.value - 1]?.description || "Completa el registro del pago.");
const paymentTutorialSteps = [
  { target: ".payment-steps", eyebrow: "Ayuda / pagos", title: "Avanza por partes", body: "El registro se divide en relación, movimiento y confirmación. Los pasos completados permanecen disponibles para corregir información." },
  { target: ".payment-step-shell", eyebrow: "Ayuda / pagos", title: "Completa sólo lo visible", body: "Cada pantalla reúne campos relacionados. Los mensajes debajo de cada campo indican qué falta antes de continuar." },
  { target: ".payment-step-shell .app-field-help__trigger", eyebrow: "Ayuda / campos", title: "Usa los signos de pregunta", body: "Pulsa ? junto al nombre de cualquier campo para conocer su propósito y qué información debes capturar." },
  { target: ".payment-form .modal-actions", eyebrow: "Ayuda / navegación", title: "Revisa antes de guardar", body: "Usa Anterior y Continuar para moverte. El sistema sólo guarda cuando confirmas el último paso." },
];
...
const reviewClient = computed(() => clientes.value.find((item) => String(item.id_cliente) === String(payment.value.id_cliente))?.nombre || clientSearch.value || "Sin cliente");
const reviewEmployee = computed(() => employees.value.find((item) => String(item.id_usuario) === String(payment.value.id_atendio))?.nombre || employeeSearch.value || "Sin responsable");
const netAmount = computed(() => (Number(payment.value.cobramos) || 0) - (Number(payment.value.pagamos) || 0));
const formattedSelectedDate = computed(() => new Intl.DateTimeFormat("es-MX", { dateStyle: "long", timeStyle: "short" }).format(selectedDate.value));
...
function onClientInput(value: string) { payment.value.id_cliente = findClient(value)?.id_cliente || ""; if (payment.value.id_cliente) delete errors.value.cliente; }
function onEmployeeInput(value: string) { payment.value.id_atendio = findEmployee(value)?.id_usuario || ""; if (payment.value.id_atendio) delete errors.value.atendio; }
...
function onKeydown(event: KeyboardEvent) { if (event.key === "Escape" && !paymentHelpOpen.value) close(); }

onMounted(async () => {
  document.addEventListener("keydown", onKeydown);
  document.body.classList.add("modal-open");
  const [clientResult, employeeResult] = await Promise.allSettled([
    loadProgressively<Payment>({ pageSize: 40, fetchPage: async (page) => await cs.getClientes(page) as Payment[], onUpdate: (items) => { clientes.value = items; } }),
    us.getUsuarios(),
  ]);
...
function errorsForStep(step: PaymentStep): Record<string, string> {
  ...
  if (step === 2) {
    if (payment.value.cobramos === "" || payment.value.cobramos == null || Number(payment.value.cobramos) < 0) next.cobramos = "Indica un monto válido.";
    if (payment.value.pagamos === "" || payment.value.pagamos == null || Number(payment.value.pagamos) < 0) next.pagamos = "Indica un monto válido.";
    if (!selectedDate.value || Number.isNaN(selectedDate.value.getTime())) next.fecha = "Selecciona fecha y hora.";
  }
  return next;
}

async function validateStep(step: PaymentStep): Promise<boolean> {
  const keys = step === 1 ? ["cliente", "asunto", "atendio"] : step === 2 ? ["cobramos", "pagamos", "fecha"] : [];
  const retained = Object.fromEntries(Object.entries(errors.value).filter(([key]) => !keys.includes(key)));
...
async function nextStep(): Promise<void> {
  if (currentStep.value < 3 && await validateStep(currentStep.value)) currentStep.value = (currentStep.value + 1) as PaymentStep;
}
