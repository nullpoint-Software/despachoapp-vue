...
const fiscalTutorialSteps = [
  {
    target: '.fiscal-hero',
    eyebrow: 'Fiscal / inicio',
    title: 'Organiza el periodo de trabajo',
    body: 'Selecciona cliente, ejercicio y mes antes de importar o revisar comprobantes.'
  },
  {
    target: '.filter-strip',
    eyebrow: 'Fiscal / filtros',
    title: 'Acota la consulta',
    body: 'Los filtros controlan los CFDI, reportes y datos usados en la conciliación.'
  },
  {
    target: '.sat-mass-download-launch',
    eyebrow: 'Fiscal / SAT',
    title: 'Descarga los CFDI',
    body: 'Conecta la e.firma con el servicio oficial del SAT. La aplicación descarga e importa automáticamente los XML del periodo.'
  },
  {
    target: '.bank-reconciliation-launch',
    eyebrow: 'Fiscal / banco',
    title: 'Compara el banco con tus reportes',
    body: 'Abre la conciliación para cargar el estado de cuenta del periodo. Dentro encontrarás una guía específica para preparar la comparación y leer sus resultados.'
  },
  {
    target: '.xml-dropzone',
    eyebrow: 'Fiscal / XML',
    title: 'Importa los comprobantes',
    body: 'Arrastra XML o el ZIP descargado del SAT. La aplicación extrae, valida y relaciona los documentos con el cliente elegido.'
  },
  {
    target: '.fiscal-tabs',
    eyebrow: 'Fiscal / dirección',
    title: 'Separa ingresos y egresos',
    body: 'Cambia entre CFDI emitidos y recibidos para revisar impuestos y seleccionar documentos.'
  },
  {
    target: '.invoice-panel',
    eyebrow: 'Fiscal / reportes',
    title: 'Revisa y prepara el reporte',
    body: 'Selecciona comprobantes válidos, consulta su detalle y abre los reportes guardados cuando necesites retomarlos.'
  }
]
...
const mxn = (invoice: Invoice, value: any) =>
  signed(invoice, value) * (invoice.moneda === 'MXN' ? 1 : num(invoice.tipo_cambio) || 1)
...
const selectedSubtotal = computed(() =>
  invoices.value.reduce(
    (total, invoice) =>
      selectedIds.has(Number(invoice.id)) ? total + mxn(invoice, invoice.subtotal) : total,
    0
  )
)
...
const counterpartName = (invoice: Invoice) =>
  invoice.direccion === 'emitida' ? invoice.receptor_nombre : invoice.emisor_nombre
...
const paymentLabel = (invoice: Invoice) =>
  invoice.forma_pago
    ? `${invoice.forma_pago} - ${paymentForms[invoice.forma_pago] || 'Otra forma'}`
    : 'Sin forma de pago'
function isSelectable(invoice: Invoice) {
  return Number(invoice.seleccionable_reporte) !== 0
}
const isPaymentComplement = (invoice: Invoice) => invoice.tipo_comprobante === 'P'
const hasFiscalAttention = (invoice: Invoice) =>
  invoice.tipo_comprobante === 'E' || num(invoice.base_iva_0) > 0 || num(invoice.base_exento) > 0
type PaymentReference = { invoiceId: number | null; uuid: string; folio: string; amount: number }
function paymentReferences(invoice: Invoice): PaymentReference[] {
  return String(invoice.facturas_relacionadas || '')
    .split('||')
    .filter(Boolean)
    .map((entry) => {
      const [invoiceId, uuid, folio, amount] = entry.split('|')
      return { invoiceId: Number(invoiceId) || null, uuid, folio, amount: num(amount) }
    })
}
function paymentReferenceText(invoice: Invoice, compact = false) {
  return paymentReferences(invoice)
    .map((reference) => {
      const document = reference.folio || shortUuid(reference.uuid)
      return compact ? document : `${document} · ${money(reference.amount)}`
    })
    .join('; ')
}
const invoiceGroups = computed(() => {
  const regular = invoices.value.filter((invoice) => !isPaymentComplement(invoice))
  const complements = invoices.value.filter(isPaymentComplement)
  return [
    {
      key: 'cfdi',
      label: filters.direction === 'emitida' ? 'CFDI emitidos' : 'CFDI recibidos',
      invoices: regular
    },
    { key: 'payments', label: 'Complementos de pago', invoices: complements }
...
const matchingReport = computed(() =>
  reports.value.find(
    (report) =>
      ...
      (reportDraft.type === 'anual' || Number(report.mes) === Number(reportDraft.month))
  )
)
...
function toggleAll() {
  allVisibleSelected.value
    ? selectableInvoices.value.forEach((invoice) => selectedIds.delete(Number(invoice.id)))
    : selectVisible()
}
...
function addFiles(files: File[]) {
  const accepted = files.filter((file) => /\.(xml|zip)$/i.test(file.name))
  for (const file of accepted) {
    if (!pendingFiles.value.some((item) => item.name === file.name && item.size === file.size))
      pendingFiles.value.push(file)
  }
  if (accepted.length !== files.length)
    setNotice('Sólo se agregaron archivos XML o ZIP del SAT.', 'error')
}
function openFilePicker() {
  if (!filters.clienteId) {
    setNotice('Selecciona un cliente antes de agregar XML o ZIP.', 'error')
    return
  }
    ...
    await loadProgressively<Invoice>({
      ...
      onUpdate: (items) => {
        if (version === invoiceLoadVersion) {
          ...
          items
            .filter((invoice) => !isSelectable(invoice))
            .forEach((invoice) => selectedIds.delete(Number(invoice.id)))
        }
      },
      onBackgroundError: (error: any) => {
        if (version === invoiceLoadVersion)
          setNotice(error.response?.data?.error || 'No se completó la carga de facturas.', 'error')
      }
    })
  } catch (error: any) {
    setNotice(error.response?.data?.error || 'No se pudieron cargar las facturas.', 'error')
  } finally {
    loading.value = false
  }
}
...
async function uploadQueued() {
  ...
  try {
    ...
    const ok = response.results.filter((item: any) => item.status === 'imported').length
    const duplicate = response.results.filter((item: any) => item.status === 'duplicate').length
    const errors = response.results.filter((item: any) => item.status === 'error')
    pendingFiles.value = []
    ...
    setNotice(error.response?.data?.error || 'No se pudieron importar los XML o el ZIP.', 'error')
  } finally {
...
function suggestedReportName() {
  const movement = reportDraft.direction === 'emitida' ? 'Emitidas' : 'Recibidas'
...
const reportValidation = computed(() => {
  if (!selectedIds.size) return 'Selecciona al menos un CFDI para guardar el reporte.'
  ...
  const outsidePeriod = selected.some((invoice) => {
    const period = invoicePeriod(invoice)
    return (
      !period || period.year !== year || (reportDraft.type === 'mensual' && period.month !== month)
    )
  })
  if (outsidePeriod)
  ...
  if (selected.some((invoice) => invoice.direccion !== reportDraft.direction))
...
function openReportDialog() {
  ...
  reportDraft.month = selectedMonth > 0 ? selectedMonth : now.getMonth() + 1
  syncReportName()
...
watch(
  ...
  (type) => {
    if (
      ...
      diotDraft.value.tipoOperacion = availableOperationTypes.value[0]?.value || '85'
  }
)
async function saveReport() {
  ...
  try {
    ...
    setNotice(response.message || 'Reporte guardado. Las facturas quedaron marcadas.')
    await Promise.all([loadInvoices(), loadReports()])
  } catch (error: any) {
    setNotice(error.response?.data?.error || 'No se pudo guardar el reporte.', 'error')
  } finally {
...
async function loadReport(id: number) {
  try {
    ...
    selectedIds.clear()
    activeReportInvoiceIds.clear()
    data.invoices.forEach((invoice: Invoice) => activeReportInvoiceIds.add(Number(invoice.id)))
    data.invoices
      .filter(isSelectable)
      .forEach((invoice: Invoice) => selectedIds.add(Number(invoice.id)))
    await Promise.all([loadInvoices(), loadReports()])
    activeReportId.value = id
    showReportsDrawer.value = false
    setNotice(`Corte “${data.report.nombre}” cargado con ${data.invoices.length} facturas.`)
  } catch (error: any) {
    setNotice(error.response?.data?.error || 'No se pudo abrir el reporte.', 'error')
  }
}
async function downloadReport(report: any) {
  try {
    ...
    setNotice(error.response?.data?.error || 'No se pudo descargar el reporte.', 'error')
  }
}
...
async function openProvider(invoice: Invoice) {
  ...
  try {
    const provider = await fs.getProvider(Number(filters.clienteId), invoice.emisor_rfc)
    diotDraft.value = {
...
async function saveProvider() {
  try {
    await fs.updateProvider(Number(filters.clienteId), diotDraft.value.rfc, diotDraft.value)
    showDiotDialog.value = false
    setNotice('La configuración se aplicó a todas las facturas de este proveedor.')
    await loadInvoices()
  } catch (error: any) {
    setNotice(
      error.response?.data?.error || 'No se pudo guardar la configuración del proveedor.',
      'error'
    )
  }
}
...
function reportRows(items: Invoice[]) {
  const regular = items.filter((invoice) => !isPaymentComplement(invoice))
  const complements = items.filter(isPaymentComplement)
  ...
  const rows = (group: Invoice[]) =>
    group.map((invoice: Invoice) => {
      const payment =
        isPaymentComplement(invoice) && paymentReferences(invoice).length
      ...
      return hasFiscalAttention(invoice)
        ? values.map((content) => ({
            content,
            styles: { fillColor: [255, 244, 188], textColor: [36, 35, 33] }
          }))
        : values
    })
  if (!complements.length) return rows(regular)
  return [
    section('COMPROBANTES FISCALES', regular.length),
    ...rows(regular),
    section('COMPLEMENTOS DE PAGO', complements.length),
    ...rows(complements)
  ]
}
function reportTotal(items: Invoice[], field: string) {
  return items.reduce((total, invoice) => total + mxn(invoice, invoice[field]), 0)
}
...
async function buildReportPdf(dataSets: any[], title: string) {
  ...
  dataSets.forEach((data, index) => {
    ...
    const startY = index === 0 ? 34 : reportTableFits(data, sharedPageStart) ? sharedPageStart : 34
...
async function previewMonthlyPdf(month: number) {
  try {
    const monthlyReports = [...reportsFor(month, 'mensual')].sort((a: any, b: any) =>
    ...
    await buildReportPdf(dataSets, `Reporte mensual ${monthNames[month - 1]} ${filters.year}`)
  } catch {
...
async function removeInvoice(invoice: Invoice) {
  ...
  try {
    ...
    setNotice(error.response?.data?.error || 'No se pudo eliminar el CFDI.', 'error')
  }
}
async function exportReportWorkbook(data: any) {
  ...
  sheet.getCell(3, 13).value =
    `Movimiento: ${data.report.direccion === 'emitida' ? 'Ingresos' : 'Egresos'}`
  ...
  const groupedInvoices = [
    {
      ...
      invoices: data.invoices.filter((invoice: Invoice) => !isPaymentComplement(invoice))
    },
    { label: 'COMPLEMENTOS DE PAGO', invoices: data.invoices.filter(isPaymentComplement) }
  ].filter((group) => group.invoices.length)
  let visualIndex = 0
  groupedInvoices.forEach((group) => {
    const sectionRow = sheet.addRow([`${group.label} · ${group.invoices.length}`])
    sheet.mergeCells(sectionRow.number, 1, sectionRow.number, lastColumn)
    sectionRow.height = 22
    sectionRow.getCell(1).font = { name: 'Arial', size: 9, bold: true, color: { argb: 'FFFFFFFF' } }
    sectionRow.getCell(1).fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'FF242321' }
    }
    sectionRow.getCell(1).alignment = { vertical: 'middle', horizontal: 'left' }
    group.invoices.forEach((invoice: Invoice) => {
      const exchangeRate = invoice.moneda === 'MXN' ? 1 : num(invoice.tipo_cambio) || 1
      const row = sheet.addRow([
        invoice.uuid,
        invoice.direccion,
        invoice.fecha_emision,
        typeLabel(invoice.tipo_comprobante),
        counterpartRfc(invoice),
        counterpartName(invoice),
        invoice.forma_pago || '',
        invoice.metodo_pago || '',
        ...
        num(invoice.base_iva_16),
        num(invoice.iva_16),
        num(invoice.base_iva_8),
        num(invoice.iva_8),
        num(invoice.base_iva_0),
        num(invoice.base_exento),
        num(invoice.base_no_objeto),
        num(invoice.iva_retenido),
        num(invoice.isr_retenido),
        num(invoice.total),
        num(invoice.total) * exchangeRate,
        invoice.productos || '',
        invoice.advertencia_reporte || '',
        paymentReferenceText(invoice)
      ])
      row.height = 30
      for (let column = 1; column <= lastColumn; column += 1) {
        const cell = row.getCell(column)
        cell.font = { name: 'Arial', size: 9, color: { argb: 'FF242321' } }
        cell.alignment = { vertical: 'middle', wrapText: column >= 23 }
        const fill = hasFiscalAttention(invoice)
          ? 'FFFFF4BC'
          : visualIndex % 2 === 1
            ? 'FFF4F2EC'
            : null
  ...
  sheet.columns.forEach((column, index) => {
    const preferred = [
      39, 12, 19, 14, 18, 32, 14, 14, 10, 13, 15, 15, 14, 15, 14, 14, 14, 14, 14, 14, 15, 16, 42,
      42, 46
    ][index]
    column.width = preferred
  })
  ;[23, 24, 25].forEach((column) => {
  ...
  const safeName = String(data.report.nombre || `Reporte_${data.report.id}`).replace(
    /[\\/:*?"<>|]+/g,
    '-'
  )
  saveAs(
...
async function removeInvoiceFromReport(invoice: Invoice) {
  ...
  if (!confirmed) return
  try {
    const response = await fs.removeReportInvoice(activeReportId.value, Number(invoice.id))
    selectedIds.delete(Number(invoice.id))
    activeReportInvoiceIds.delete(Number(invoice.id))
    if ((response.deletedReportIds || []).includes(activeReportId.value)) closeActiveReport()
    setNotice(response.message || 'Factura retirada del reporte.')
    await Promise.all([loadInvoices(), loadReports()])
  } catch (error: any) {
    setNotice(error.response?.data?.error || 'No se pudo quitar la factura del reporte.', 'error')
  }
}
async function removeReport(report: any) {
  const removesLinkedDiot = report.tipo === 'mensual' && report.direccion === 'recibida'
  const confirmed = await confirmDialog({
  ...
  try {
    ...
    if (activeReportId.value && deletedIds.includes(activeReportId.value)) closeActiveReport()
    setNotice(response.message || 'Reporte eliminado.')
    await Promise.all([loadInvoices(), loadReports()])
  } catch (error: any) {
    setNotice(error.response?.data?.error || 'No se pudo eliminar el reporte.', 'error')
  }
}
async function downloadXml(invoice: Invoice) {
  try {
    ...
    setNotice(error.response?.data?.error || 'No se pudo descargar el XML.', 'error')
  }
}

function onGlobalKeydown(event: KeyboardEvent) {
  if (event.key === 'Escape' && showReportsDrawer.value) showReportsDrawer.value = false
}
onMounted(async () => {
