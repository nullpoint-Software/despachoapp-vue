...
interface LogsModalProps { visible: boolean }
...
interface TypeOption { label: string; value: OperationSuffix }
interface RouteScope { title: string; prefix: string }
...
const filteredLogs = computed(() => {
  ...
  return logs.value.filter((log) => {
    ...
    const suffix = String(log.type || "").match(/Added|Updated|Deleted/)?.[0] as OperationSuffix | undefined;
