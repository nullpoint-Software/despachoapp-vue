...
interface PaymentColumn { field: string; header: string }
interface GlobalFilter { value: string | null; matchMode: "contains" }
...
function openDateRangeModal() { showDateRangeModal.value = true; }
function closeDateRangeModal() { showDateRangeModal.value = false; }
...
function applyFilters() {
  if (draftPeriodFilter.value === "personalizado" && draftDateFrom.value > draftDateTo.value) {
    toast.add({ severity: "warn", summary: "Periodo inválido", detail: "La fecha inicial debe ser anterior a la fecha final.", life: 3000 });
    return;
  }
...
const normalizePaymentPage = (items: ConceptPayment[]): ConceptPayment[] => items.map((item) => ({
  ...item,
...
async function loadPaymentsPage() {
  ...
  try {
    ...
    const filtered = normalized.filter((payment) => {
      ...
      const matchesMovement = movementFilter.value === "todos"
      ...
      const matchesPeriod = periodFilter.value === "todos"
