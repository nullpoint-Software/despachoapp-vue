...
const movementLabel = (type: "credit" | "debit") =>
  type === "credit" ? "Abono" : "Cargo";
...
async function openDialog() {
  ...
  show.value = true;
  await nextTick();
  if (!localStorage.getItem("tourBankReconciliationSetupDone")) reconciliationTutorialOpen.value = true;
}
...
async function runComparison() {
  ...
  try {
    ...
    errorMessage.value =
      ...
      "No se pudo extraer y comparar el estado de cuenta.";
  } finally {
    loading.value = false;
  }
}
...
function exportFindings() {
  ...
  const headers = [
    "Estado", "Explicación", "Fecha banco", "Tipo banco", "Concepto banco",
    "Referencia bancaria", "Importe banco", "Fecha reporte", "Contraparte",
    "RFC", "UUID", "Importe reporte", "Reporte",
  ];
