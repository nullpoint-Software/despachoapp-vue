...
const canvasNotes = computed(() => notes.value.filter((n: Note) => n.status === 'canvas'))
const storageNotes = computed(() => notes.value.filter((n: Note) => n.status === 'storage'))
...
export function useNotesStore() {
  ...
  const fetchNotes = async (): Promise<void> => {
    ...
    try {
      ...
      const normalizeNotes = (items: any[]) =>
        items.map((raw) => {
      ...
      await loadProgressively<any>({
        ...
        onBackgroundError: (backgroundError) =>
          console.error('No se completó la carga de notas', backgroundError)
      })
      // ----------------------
    } catch (e) {
      const err = e as any
  ...
  const pinnedNotes = computed(() => notes.value.filter((note) => Boolean(note.pinned)))
