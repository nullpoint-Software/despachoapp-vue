...
export function markdownToVisualHtml(source = "") {
  ...
  template.content.querySelectorAll("img").forEach(image => {
    ...
    const isOnlyChild = parent?.tagName === "P"
...
function serializeInline(node: Node): string {
  ...
  if (tag === "U" || element.style.textDecoration.includes("underline") || element.style.textDecorationLine.includes("underline")) return `<u>${content()}</u>`;
  ...
  if (tag === "SUP" || tag === "SUB" || tag === "MARK") return `<${tag.toLowerCase()}>${content()}</${tag.toLowerCase()}>`;
  return content();
}
...
function serializeList(list: HTMLElement, depth = 0): string {
  ...
  return items.map((item, index) => {
    ...
    const checkbox = Array.from(item.children).find(child => child.matches("input[type='checkbox']")) as HTMLInputElement | undefined;
    const body = Array.from(item.childNodes)
      .filter(child => !(child.nodeType === 1 && ((child as Element).matches("ul,ol,input[type='checkbox']"))))
      .map(serializeInline)
      .join("")
      .trim();
    ...
    const nestedText = nested
      .map(child => serializeList(child, depth + 1).trimEnd().split("\n").map(line => `  ${line}`).join("\n"))
      .filter(Boolean)
      .join("\n");
...
function serializeTable(table: HTMLElement) {
  ...
  const normalized = values.map(row => Array.from({ length: columns }, (_, index) => row[index] || ""));
...
function serializeBlock(node: Node): string {
  if (node.nodeType === 3) return node.textContent?.trim() ? `${escapeMarkdownText(node.textContent)}\n\n` : "";
  if (node.nodeType !== 1) return "";
  const element = node as HTMLElement;
  const tag = element.tagName;
  ...
  if (tag === "BLOCKQUOTE") {
    const quote = Array.from(element.childNodes).map(child => child.nodeType === 1 ? serializeBlock(child) : serializeInline(child)).join("").trim();
  ...
  if (tag === "FIGURE") {
    ...
    const caption = element.querySelector("figcaption")?.textContent?.trim() || image.getAttribute("alt") || "Descripción de la imagen";
  ...
  if (tag === "DIV" || tag === "SECTION" || tag === "ARTICLE") {
    const hasBlockChildren = Array.from(element.children).some(child => /^(P|DIV|H[1-6]|UL|OL|BLOCKQUOTE|PRE|TABLE|FIGURE|HR)$/.test(child.tagName));
