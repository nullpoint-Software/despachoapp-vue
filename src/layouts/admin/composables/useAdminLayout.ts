...
export function useAdminLayout() {
  ...
  function syncOverlayTopOffset(): void {
    document.documentElement.style.setProperty(
      '--admin-overlay-top',
      isMobile.value ? '0px' : '5rem'
    )
  }

  function openNotesFromMoreMenu(): void {
    openNotes()
    closeMoreMenu()
  }
  function openLogsFromMoreMenu(): void {
    openLogs()
    closeMoreMenu()
  }
  ...
  watch(isLogsModalOpen, (isOpen) => {
    if (isOpen) logsRenderKey.value += 1
  })
  ...
  return {
    ...
    navigationItems,
    openLogs,
    openLogsFromMoreMenu,
    openNotes,
    openNotesFromMoreMenu,
