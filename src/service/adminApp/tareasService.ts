...
function normalizeEstado(value: unknown): TareaEstado {
  const normalized = String(value ?? 'Disponible')
    .trim()
    .toLocaleLowerCase('es-MX')
  if (normalized === 'pendiente') return 'Pendiente'
  if (['en progreso', 'en proceso', 'progreso'].includes(normalized)) return 'En Progreso'
  if (['terminado', 'terminada', 'completado', 'completada'].includes(normalized))
    return 'Terminado'
  return 'Disponible'
}
...
function normalizeTareaCollection(payload: unknown, visited = new Set<unknown>()): TareaDto[] {
  ...
  if (Array.isArray(payload))
    return payload.map(normalizeTarea).filter((task): task is TareaDto => task !== null)
