...
export interface ComplianceScheduleRunResult {
  total: number
  completed: number
  failed: number
...
export default class CumplimientoService {
  ...
  async getOpiniones(params?: { search?: string; status?: string }) {
    return (await this.axios.get(`${this.serverip}/cumplimiento`, { params }))
      .data as ComplianceResponse
  }
  ...
  async getProgramacion() {
    return (await this.axios.get(`${this.serverip}/cumplimiento/schedule`))
      .data as ComplianceScheduleConfig
  }
  ...
  async ejecutarProgramacionAhora(regimes: string[]) {
    return (
      await this.axios.post(
        `${this.serverip}/cumplimiento/schedule/run`,
        { regimes },
        { timeout: 0 }
      )
    ).data as {
      result: ComplianceScheduleRunResult
      config: ComplianceScheduleConfig
    }
  }
  ...
  async iniciarSesionTerceros() {
    return (
      await this.axios.post(`${this.serverip}/cumplimiento/terceros/sessions`, undefined, {
        timeout: 15000
      })
    ).data as ThirdPartySessionState
  }

  async obtenerSesionTerceros(sessionId: string) {
    return (
      await this.axios.get(`${this.serverip}/cumplimiento/terceros/sessions/${sessionId}`, {
        params: { _ts: Date.now() },
  ...
  async iniciarAccesoTerceros(
    sessionId: string,
    credentials: { rfc: string; password: string; captcha: string }
  ) {
    return (
      await this.axios.post(
        `${this.serverip}/cumplimiento/terceros/sessions/${sessionId}/login`,
        credentials,
        { timeout: 15000 }
      )
    ).data as ThirdPartySessionState
  ...
  async recargarSesionTerceros(sessionId: string) {
    return (
      await this.axios.post(
        `${this.serverip}/cumplimiento/terceros/sessions/${sessionId}/reload`,
        undefined,
        { timeout: 10000 }
      )
    ).data as ThirdPartySessionState
  ...
  async enviarEntradaTerceros(sessionId: string, input: ThirdPartyInput) {
    return (
      await this.axios.post(
        `${this.serverip}/cumplimiento/terceros/sessions/${sessionId}/input`,
        input,
        { timeout: 10000 }
      )
    ).data as ThirdPartySessionState
  ...
  async sincronizarTerceros(sessionId: string, options: { offset: number; limit: number }) {
    return (
      await this.axios.post(
        `${this.serverip}/cumplimiento/terceros/sessions/${sessionId}/sync`,
        options
      )
    ).data as {
      results: ComplianceRecord[]
